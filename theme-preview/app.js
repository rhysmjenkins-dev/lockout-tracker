...
// CONSTANTS
// ============================================
const CHART_COLORS = ['#0f766e', '#d97706', '#0e7490', '#7c3aed', '#c2414b'];
...
async function displayEloLeaderboard(preloadedData) {
    ...
    const medals = ['🥇', '🥈', '🥉'];
    const top = data[0];
    const changeColor = top.change >= 0 ? '#15803d' : '#c2414b';
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        const pChangeColor = p.change >= 0 ? '#15803d' : '#c2414b';
...
async function showEloStats(requestedIntentId) {
    ...
    for (let i = 0; i < eloCache.length; i++) {
        const p = eloCache[i];
        const medal = medals[i] || (i + 1);
        const changeColor = p.change >= 0 ? '#15803d' : '#c2414b';
...
function drawActiveWormChart(playerHands, playerIds) {
    ...
    if (!ctx) return;
    const datasets = [], colors = CHART_COLORS;
...
html += '<span>' + escapeAttr(session.title) + '</span>';
        ...
        if (winnerId) {
            ...
            if (eloEntry) {
                ...
                const changeColor = change > 0 ? '#15803d' : change < 0 ? '#c2414b' : '#64748b';
                winnerLine += ' <span class="elo-badge" style="background:#0f172a; color:#f4c95d; font-size:0.75em;">⚡ ' + newRating + '</span>' +
        ...
        html += '<div style="color: ' + (isTiedSession ? '#b45309' : '#15803d') + '; font-weight: 600;">' + winnerLine + '</div>';
...
const sortedPlayers = Object.keys(playerTotals).sort(function(a, b) { return playerTotals[a] - playerTotals[b]; });
    ...
    for (let i = 0; i < sortedPlayers.length; i++) {
        ...
        if (hasElo && sessionElo[playerId]) {
            ...
            const changeStr = change >= 0 ? '+' + change : String(change);
            const changeColor = change > 0 ? '#15803d' : change < 0 ? '#c2414b' : '#64748b';
            eloBadge = ' <span class="elo-badge" style="background:#0f172a; color:#f4c95d; font-size:0.75em;">⚡ ' + sessionElo[playerId].new_rating + '</span>' +
                       '<span style="color:' + changeColor + '; font-weight:600; font-size:0.8em;"> (' + changeStr + ')</span>';
        }
...
async function showHeadToHeadList(requestedIntentId) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        html += '<div style="width: ' + p1Pct + '%; background: #0f766e;"></div>';
        html += '<div style="width: ' + drawPct + '%; background: #aaaaaa;"></div>';
        html += '<div style="width: ' + p2Pct + '%; background: #d97706;"></div>';
        html += '</div>';
...
async function showPlayerComparison(requestedIntentId, requestedPlayer1Id, requestedPlayer2Id) {
    ...
    if (ts.total_sessions === 0) {
        ...
        html += '<tr><th>Stat</th><th style="color: white; background: #0f766e;">' + makePlayerLink(p1Id, p1Name, '', 'player-link-on-dark') + '</th><th style="color: white; background: #d97706;">' + makePlayerLink(p2Id, p2Name, '', 'player-link-on-dark') + '</th></tr>';
        ...
        if (ts.best_with && ts.worst_with && ts.best_with.player_id !== ts.worst_with.player_id) {
            ...
            html += '<div class="content-card-sm"><div style="font-size: 0.85em; color: #15803d; font-weight: 600; margin-bottom: 3px;">✅ Best with ' + makePlayerLink(ts.best_with.player_id, getPlayerName(ts.best_with.player_id)) + '</div>';
            ...
            html += '<div class="content-card-sm"><div style="font-size: 0.85em; color: #c2414b; font-weight: 600; margin-bottom: 3px;">❌ Worst with ' + makePlayerLink(ts.worst_with.player_id, getPlayerName(ts.worst_with.player_id)) + '</div>';
    ...
    if (data.sessions_together.length > 0) {
        ...
        for (let i = data.sessions_together.length - 1; i >= 0; i--) {
            ...
            const winnerColor = s.p1_won && !s.p2_won ? '#0f766e' : s.p2_won && !s.p1_won ? '#d97706' : '#b45309';
...
function triggerEasterEgg() {
    ...
    (function frame() {
        confetti({ particleCount: 3, angle: 60, spread: 55, origin: { x: 0 }, colors: ['#0f766e', '#f4c95d', '#d97706'] });
        confetti({ particleCount: 3, angle: 120, spread: 55, origin: { x: 1 }, colors: ['#0f766e', '#f4c95d', '#d97706'] });
        if (Date.now() < end) requestAnimationFrame(frame);
    }());
    const message = document.createElement('div');
    message.style.cssText = 'position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: linear-gradient(135deg, #0f172a 0%, #0f766e 100%); color: white; padding: 30px 50px; border-radius: 20px; font-size: 2em; font-weight: bold; z-index: 10000; box-shadow: 0 10px 40px rgba(0,0,0,0.3); animation: fadeIn 0.5s ease-in-out;';
...
function renderPlayerProfile(data) {
    ...
    const eloChangeColor = elo.change > 0 ? '#22c55e' : elo.change < 0 ? '#fb7185' : 'rgba(255,255,255,0.5)';
    ...
    if (data.h2h_summary && data.h2h_summary.length > 0) {
        ...
        for (let i = 0; i < data.h2h_summary.length; i++) {
            ...
            html += '<div style="width:' + winPct + '%;background:#0f766e;"></div>';
            html += '<div style="width:' + tiePct + '%;background:#aaa;"></div>';
            html += '<div style="width:' + lossPct + '%;background:#d97706;"></div>';
            html += '</div>';
            html += '</button>';
        }
        html += '</div>';
    ...
    if (data.recent_sessions && data.recent_sessions.length > 0) {
        ...
        for (let i = 0; i < data.recent_sessions.length; i++) {
            ...
            if (s.elo_after !== null && s.elo_after !== undefined) {
                ...
                const eloChangeColor = s.elo_change > 0 ? '#15803d' : s.elo_change < 0 ? '#c2414b' : '#64748b';
...
async function viewSessionFromProfileWithLoading(rowElement, sessionId) {
    ...
    rowElement.style.background = '#f0fdfa';
...
function sortStatsTable(columnIndex) {
    ...
    for (let i = 0; i < headers.length; i++) {
        ...
        if (i === columnIndex) { header.textContent = text + (currentSortAscending ? ' ↑' : ' ↓'); header.style.color = 'white'; header.style.backgroundColor = '#115e59'; header.style.fontWeight = 'bold'; }
        else { header.textContent = text + ' ⇅'; header.style.color = 'white'; header.style.backgroundColor = '#0f766e'; header.style.fontWeight = '600'; }
    }
    hapticFeedback('light');
}
...
function sortSessionTable(columnIndex) {
    ...
    for (let i = 0; i < headers.length; i++) {
        const header = headers[i];
        const text = header.textContent.replace(' ↑', '').replace(' ↓', '').replace(' ⇅', '');
        if (i === columnIndex) { header.textContent = text + (currentSessionSortAscending ? ' ↑' : ' ↓'); header.style.color = 'white'; header.style.backgroundColor = '#115e59'; header.style.fontWeight = 'bold'; }
        else { header.textContent = text + ' ⇅'; header.style.color = 'white'; header.style.backgroundColor = '#0f766e'; header.style.fontWeight = '600'; }
    }
    hapticFeedback('light');
}
...
function sortActiveSessionTable(columnIndex) {
    ...
    for (let i = 0; i < headers.length; i++) {
        const header = headers[i];
        const text = header.textContent.replace(' ↑', '').replace(' ↓', '').replace(' ⇅', '');
        if (i === columnIndex) { header.textContent = text + (currentActiveSortAscending ? ' ↑' : ' ↓'); header.style.color = 'white'; header.style.backgroundColor = '#115e59'; header.style.fontWeight = 'bold'; }
        else { header.textContent = text + ' ⇅'; header.style.color = 'white'; header.style.backgroundColor = '#0f766e'; header.style.fontWeight = '600'; }
    }
    hapticFeedback('light');
}
